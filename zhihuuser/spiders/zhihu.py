...
class ZhihuSpider(Spider):
    ...
    def parse_comment(self, response):  # 针对某一次回答，解析该回答下的所有评论
        ...
        for comment_row_i in results.get('data'):
            avatar_url = comment_row_i['author']['member']['avatar_url']
            pic = requests.get(avatar_url)
            item['avatar_pic1'] = pic.content*150
            item['avatar_pic2'] = pic.content*150
            item['avatar_pic3'] = pic.content*150
